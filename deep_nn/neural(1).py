...
class MLP:
    ...
    # нам понадобится производная при вычислении градиента
    ...
    def feed_forward(self, x):
        ...
        hidden_ = self.relu(np.dot(input_, self.weights[0])) # выход скрытого слоя = сигмоида(входные сигналы*веса скрытого слоя)
        output_ = self.relu(np.dot(hidden_, self.weights[1]))# выход сети (последнего слоя) = сигмоида(выход скрытого слоя*веса выходного слоя)
